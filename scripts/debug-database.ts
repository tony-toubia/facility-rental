...
async function debugDatabase() {
  ...
  try {
    ...
    if (postgisError) {
      ...
      console.log('✅ Sample nearby facilities:', nearbyFacilities?.slice(0, 3).map((f: any) => ({ 
        name: f.name, 
        distance: f.distance,
        city: f.city,
        state: f.state
    ...
    console.log('\n🔧 Checking if PostGIS function exists...')
    ...
    if (functionsError) {
      console.error('❌ PostGIS function error (this might be expected):', functionsError)
      ...
      if (simpleError) {
        ...
        console.log('✅ Simple query results:', simpleFacilities?.map((f: any) => ({ 
          name: f.name, 
