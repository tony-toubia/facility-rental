...
-- Updated for the actual database schema (direct relationships, not junction tables)
...
-- Note: This table has facility_id column (direct relationship)
...
CREATE POLICY "allow_owner_write_amenities" ON public.facility_amenities
    FOR ALL USING (
        auth.uid() IS NOT NULL AND
        EXISTS (
            SELECT 1 FROM facility_facilities 
...
CREATE POLICY "allow_owner_write_features" ON public.facility_features
    FOR ALL USING (
        auth.uid() IS NOT NULL AND
        EXISTS (
            SELECT 1 FROM facility_facilities 
...
-- 6. Keep facility_users restricted to authenticated users only
-- (No changes needed - users should remain private)

-- 7. Keep facility_availability restricted for now
-- (Booking-related data should require authentication)

-- 8. Keep facility_bookings restricted to authenticated users
-- (Booking data should remain private)

-- 9. Keep facility_reviews restricted to authenticated users
